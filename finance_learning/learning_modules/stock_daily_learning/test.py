...
def get_label_index(row, df, env):
    index = df.index.get_loc(row.name) + env.sequence_len
    window_df = df[index:index + env.evaluate_len]
...
def create_train_infos(db_conn, env):
    ...
    rise_train_info_indexs = train_infos.label_index == 0
    stay_train_info_indexs = train_infos.label_index == 1
    fall_train_info_indexs = train_infos.label_index == 2

    train_infos_len = min(
        rise_train_info_indexs.sum(),
        stay_train_info_indexs.sum(),
        fall_train_info_indexs.sum()
        )
    ...
    result = pd.concat((
        train_infos[rise_train_info_indexs].sample(train_infos_len),
        train_infos[stay_train_info_indexs].sample(train_infos_len),
        train_infos[fall_train_info_indexs].sample(train_infos_len)
        )).sample(frac=1)
